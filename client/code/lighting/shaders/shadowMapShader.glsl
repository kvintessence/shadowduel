...
vec4 effect(vec4 color, Image texture, vec2 texture_coords, vec2 screen_coords)
{
    ...
    for (number y = 0.0; y < resolutionY; y++) {
        ...
        vec2 norm = vec2(texture_coords.s * (canvasSize / resolutionY), y / resolutionY) * 2.0 - 1.0;
        ...
        vec4 data = Texel(texture, coord * (resolutionY / canvasSize));
